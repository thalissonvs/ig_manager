...
class InstagramManager(IManager):
    ...
    def __init__(self, android_automator: AndroidAutomator, selectors: ISelectors) -> None:
    ...
    def login(self, username: str, password: str) -> bool:
        ...
        try:
            self.android_automator.click(self.selectors.LOGIN_INTO_ANOTHER_ACCOUNT, 5)
        except Exception:
            pass
        self.android_automator.send_keys(self.selectors.USERNAME_INPUT, username)
        self.android_automator.send_keys(self.selectors.PASSWORD_INPUT, password)
    ...
    def verify_login(self) -> bool:
        login_verifier = self.android_automator.find_element(self.selectors.LOGIN_VERIFIER)
        try:
            self.android_automator.wait_for_element(login_verifier, self.DEFAULT_TIMEOUT)
            return True
        except TimeoutError:
            return False
